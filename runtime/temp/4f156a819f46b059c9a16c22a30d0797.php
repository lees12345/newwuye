<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:79:"/home/wwwroot/only.1yuaninfo.com/./application/wxadmint1/view/survey/index.html";i:1635479599;s:71:"/home/wwwroot/only.1yuaninfo.com/application/wxadmint1/view/layout.html";i:1635921773;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="utf-8" />
				...
				<div class="row">
					<div class="col-xs-12">
						<div class="table-responsive">
							<!-- 搜索框 -->
							<form class="form-search" action="<?php echo url('wxadmint1/survey/index'); ?>">
							  <input type="hidden" name="v" value="1">
								<span class="input-icon">
									<select name="is_release" class="nav-search-input">
								...
								<span class="input-icon">
									<select name="que_type" class="nav-search-input">
										<option value="">问卷类型</option>
										<?php if(is_array($que_type) || $que_type instanceof \think\Collection || $que_type instanceof \think\Paginator): $key = 0; $__LIST__ = $que_type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$que_t): $mod = ($key % 2 );++$key;?>
											<option value="<?php echo $key; ?>"><?php echo $que_t; endforeach; endif; else: echo "" ;endif; ?>
									</select>
								</span>
								...
								<?php if(is_array($lists) || $lists instanceof \think\Collection || $lists instanceof \think\Paginator): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lists): $mod = ($i % 2 );++$i;?>
									<tbody>
										<tr>
											...
											<td><a href="<?php echo url('wxadmint1/Survey/show'); ?>?id=<?php echo $lists['id']; ?>"><?php echo $lists['title']; ?></a></td>
											<td><?php echo $que_type[$lists['que_type']]; ?></td>
											<!-- <td><?php echo $lists['start_time']; ?></td>
											<td><?php echo $lists['end_time']; ?></td> -->
											<td><?php echo $lists['remark']; ?></td>
											<td><?php if($lists['is_release']==0): ?><spen style="color:red">禁用</spen><?php else: ?>启用<?php endif; ?></td>
											<!--<td></td>-->
											<td><a href="<?php echo url('wxadmint1/Survey/user_show'); ?>?id=<?php echo $lists['id']; ?>&is_sub=1"><?php if((isset($num[$lists['id']]['is_sub']))): ?><?php echo $num[$lists['id']]['is_sub']; ?>人<?php else: ?>0人<?php endif; ?></a></td>
											<td><a href="<?php echo url('wxadmint1/Survey/user_show'); ?>?id=<?php echo $lists['id']; ?>&is_sub=0"><?php if((isset($num[$lists['id']]['not_sub']))): ?><?php echo $num[$lists['id']]['not_sub']; ?>人<?php else: ?>0人<?php endif; ?></a></td>

											<td><?php echo $lists['creator']; ?></td>
											<td><?php echo $lists['create_time']; ?></td>
											<td>
												<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
													<a onclick="alert('暂不可修改问卷');{return false;}" href="<?php echo url('wxadmint1/Survey/upd'); ?>?id=<?php echo $lists['id']; ?>"><button class="btn btn-xs btn-info">
														<i class="icon-edit bigger-120">编辑</i>
													</button>
													</a>
													<a onclick="alert('暂不可删除问卷');{return false;}"  href="<?php echo url('wxadmint1/Survey/del'); ?>?id=<?php echo $lists['id']; ?>"><button class="btn btn-xs btn-danger">
														<i class="icon-trash bigger-120">删除</i>
													</button>
													</a>
													<!--<a onclick="if(!confirm('您确定要删除当前问卷调查吗')){return false;}"  href="<?php echo url('wxadmint1/Survey/del'); ?>?id=<?php echo $lists['id']; ?>"><button class="btn btn-xs btn-danger">-->
														<!--<i class="icon-trash bigger-120">删除</i>-->
													<!--</button>-->
													<!--</a>-->
													<!--<?php if($lists['is_release'] == 0): ?>-->
														<!--<a onclick="if(!confirm('您确定启用当前问卷吗？')){return false;}"  href="<?php echo url('wxadmint1/Survey/release'); ?>?id=<?php echo $lists['id']; ?>&is_release=1" ><button class="btn btn-xs btn-info" >-->
															<!--<i class="bigger-120">启用</i>-->
														<!--</button>-->
														<!--</a>-->
													<!--<?php else: ?>-->
														<!--<a onclick="if(!confirm('您确定禁用当前问卷吗？')){return false;}"  href="<?php echo url('wxadmint1/Survey/release'); ?>?id=<?php echo $lists['id']; ?>&is_release=0" ><button class="btn btn-xs" >-->
															<!--<i class="bigger-120">禁用</i>-->
														<!--</button>-->
														<!--</a>-->
														<!--<a onclick="if(!confirm('您确定推送当前问卷吗？')){return false;}"  href="<?php echo url('wxadmint1/Survey/enable'); ?>?id=<?php echo $lists['id']; ?>" ><button class="btn btn-xs btn-danger" >-->
															<!--<i class="bigger-120">推送</i>-->
														<!--</button>-->
														<!--</a>-->
													<!--<?php endif; ?>-->
												</div>
