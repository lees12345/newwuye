<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:76:"/home/wwwroot/only.1yuaninfo.com/./application/wxadmint1/view/apply/now.html";i:1635479594;s:71:"/home/wwwroot/only.1yuaninfo.com/application/wxadmint1/view/layout.html";i:1635995797;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="utf-8" />
...
<div class="main-content">
	...
	<div class="page-content">
		...
		<div class="row">
			<div class="col-xs-12">
				...
				<div class="row">
					<div class="col-xs-12">
						...
						<!-- 搜索框 -->
			<form class="form-search" action="<?php echo url('wxadmint1/apply/now'); ?>">
			  <input type="hidden" name="v" value="1">
				<span class="input-icon">
					<input type="text" placeholder="请输入用户姓名/联系方式" class="nav-search-input" id="nav-search-input" autocomplete="off" name='search'/>
					<i class="icon-search nav-search-icon"></i>
			...
			<!-- <a href="daochu.html"><input type="button" value='导出签约客户' class="btn btn-info" style="border:0;padding:4px 10px;background:#4f99c6;color:#fff;"></a> -->
			<br/>
						<span class="btn btn-sm" data-rel="tooltip" title="" data-original-title="Default">总申请条数：<?php echo $us; ?></span>

						<button onclick="sendAgree();" style="border:0;background:#0099cc;padding:3px 5px;color:#fff;">一键同意</button>
						...
						<?php if(is_array($data) || $data instanceof \think\Collection || $data instanceof \think\Paginator): $key = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($key % 2 );++$key;?>
								...
								<tbody>
									<tr>
										...
										<td><?php echo $data['apply_id']; ?></td>
										<td><?php echo $data['shen']['user_name']; ?></td>
										<td>
											<?php echo $data['shen']['user_tel']; ?>
										</td>
										<td><?php echo $data['apply_num']; ?></td>
										<td><?php echo date("Y-m-d H:i:s",$data['apply_addtime']); ?></td>
										<td>
											<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
												<a onclick="if(!confirm('您确定要删除所选申请信息吗')){return false;}"  href="<?php echo url('wxadmint1/apply/del'); ?>?apply_id=<?php echo $data['apply_id']; ?>"><button class="btn btn-xs btn-danger">
													<i class="icon-trash bigger-120"></i>
												</button>
												</a>
												<span id="upd_one"><button onclick="checked(this,'<?php echo $data['shen']['user_name']; ?>','<?php echo $data['shen']['id']; ?>','<?php echo $data['apply_num']; ?>','<?php echo $data['apply_id']; ?>')" style="border:0;background:#0099cc;padding:3px 5px;color:#fff;">同意</button></span>
												<span id="upd_two"><button onclick="check(this,'<?php echo $data['shen']['id']; ?>','<?php echo $data['shen']['user_name']; ?>','<?php echo $data['apply_num']; ?>','<?php echo $data['apply_id']; ?>')" style="border:0;background:#0099cc;padding:3px 5px;color:#fff;">不同意</button></span>
											</div>
										</td>
									</tr>
								</tbody>
...
<script>
function checked(ts,$user_name,$user_id,$user_num,$userupd_id)
{
	var user_id = $user_id;
	var user_name = $user_name;
		var user_num = $user_num;
		var userupd_id = $userupd_id;
			$.mbox({
				area: [ "450px", "auto" ], //弹框大小
				border: [ 0, .5, "#666" ],
				dialog: {
					msg: "<p style=''>用户姓名："+user_name+"</p>添加次数：<input type='text' id='num' style='width:50px;border:0;border-bottom:#f2f2f2 solid 1px' value='"+user_num+"'>",
					...
					yes: function() { 
						...
						if(num.length!='')
						{
							...
							{
								$.ajax({
									data:{'user_id':user_id,'apply_num':num,'apply_id':userupd_id},
									...
									success:function(msg)
									{
										...
										$(ts).html('已审核</button>');
										$(ts).removeAttr("style");
										$(ts).attr('style','border:0;background:#999;padding:3px 5px;color:#fff');
										$(ts).attr("disabled","true"); //设置变灰按钮  
										$('#upd_two button').attr("disabled","true");
										}else if(msg=='2')
										{
											alert('审核失败');
										}else if(msg=='3')
...
<script>
//不同意
function check(ts,$user_id,$user_name,$user_num,$userupd_id)
{
	var user_id = $user_id;
		var user_name = $user_name;
		var user_num=$user_num;
		...
		//alert(user_id);
		$.ajax({
			data:{'user_id':user_id,'user_name':user_name,'user_num':user_num,'userupd_id':userupd_id},
			url:"numno.html",
			type:'POST',
			success:function(msg)
			{
