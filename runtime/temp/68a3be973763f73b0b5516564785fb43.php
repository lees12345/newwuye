<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:82:"/home/wwwroot/only.1yuaninfo.com/./application/wxadmint1/view/admin/rule_edit.html";i:1635479594;s:71:"/home/wwwroot/only.1yuaninfo.com/application/wxadmint1/view/layout.html";i:1637143029;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="utf-8" />
...
<div class="main-container" id="main-container">
					...
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<form class="form-horizontal" role="form" action="<?php echo url('wxadmint1/admin/rule_upd'); ?>" method="post">
							<?php echo token('__hash__'); ?>
									

								<input type="hidden" name="id" value="<?php echo $res['id']; ?>">
									<div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 组名称 </label>

										<div class="col-sm-9">
											<input type="text" id="form-field-1" placeholder="请输入组名称" class="col-xs-10 col-sm-5" name='title' value="<?php echo $res['title']; ?>"/>
										</div>
									</div>

									<div class="space-4"></div>

									<table class="details col-sm-20" width="100%" cellspacing="0" cellpadding="0" border="0">
								        	<tbody>
									   <div class="form-group">
								          <tr>
									      	...
									      	<td>
								            	<input id="all" value="1" type="hidden">
								            	<input value="全选 / 取消" class="btn" id="all_checkbox" style="width:200px;" type="button">
								                * 给组分配的权限
								            </td>
									      </tr>
									      </div>
								  <tr>
								    ...
								    <td>
								        <ul class="first_checkbox" style=" margin-left:10px;">
								        		<?php if(is_array($auth1) || $auth1 instanceof \think\Collection || $auth1 instanceof \think\Paginator): $i = 0; $__LIST__ = $auth1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$auth0): $mod = ($i % 2 );++$i;?>
								                            <li><input id="box<?php echo $auth0['id']; ?>" value="1" type="hidden">
								                    <input name="rules[]" class="checkbox<?php echo $auth0['id']; ?>" id="<?php echo $auth0['id']; ?>" onclick="checkbox(<?php echo $auth0['id']; ?>)" value="<?php echo $auth0['id']; ?>" type="checkbox" >

								                    <label for="<?php echo $auth0['id']; ?>"><?php echo $auth0['title']; ?><br>

								                    <?php if(is_array($auth0['zi']) || $auth0['zi'] instanceof \think\Collection || $auth0['zi'] instanceof \think\Paginator): $i = 0; $__LIST__ = $auth0['zi'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$auth): $mod = ($i % 2 );++$i;?>
								                    <div class="two_checkbox">
								                    ┠─&nbsp;&nbsp;                         <input name="rules[]" class="checkbox<?php echo $auth0['id']; ?>" id="<?php echo $auth['id']; ?>" value="<?php echo $auth['id']; ?>" style="margin-left:20px;" type="checkbox" >
								                         <label for="<?php echo $auth['id']; ?>"><?php echo $auth['title']; ?>                        </label></div>
								                </label></li>
	...
	<script src="/public/static/admin/js/jquery-1.9.1.js"></script>
<script type="text/javascript">
	
	// $('input[type="checkbox"]').prop('checked', false);  //默认全选权限
