<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:83:"/home/wwwroot/only.1yuaninfo.com/./application/wxadmint1/view/survey/user_show.html";i:1635479600;s:71:"/home/wwwroot/only.1yuaninfo.com/application/wxadmint1/view/layout.html";i:1637143029;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="utf-8" />
	...
	<div class="page-content">
		<div class="page-header">
			<h1>
				...
				<small>
					<i class="icon-double-angle-right"></i>
					用户列表—<span style="color: red;"><?php if($is_sub == 1): ?>已提交<?php else: ?>进入未提交<?php endif; ?></span>
				</small>
			</h1>
		</div><!-- /.page-header -->
				...
				<div class="row">
					<div class="col-xs-12">
						<div class="table-responsive">
							<!-- 搜索框 -->
							<form class="form-search" action="<?php echo url('wxadmint1/survey/user_show'); ?>">
								<input type="hidden" name="is_sub" value="<?php echo $is_sub; ?>">
								<span class="input-icon">
									<input type="text" placeholder="请输入用户手机号" class="nav-search-input" id="nav-search-input" autocomplete="off" name='user_tel'/>
									<input type="text" placeholder="请输入服务人员" class="nav-search-input" id="nav-search-input" autocomplete="off" name='user_server_people'/>
									<i class="icon-search nav-search-icon"></i>
								</span>
								...
								<?php if(is_array($lists) || $lists instanceof \think\Collection || $lists instanceof \think\Paginator): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lists): $mod = ($i % 2 );++$i;?>
									<tbody>
										<tr>
											...
											<td>
												<?php if($adminid['group_id'] == 51): if(in_array(($lists['type_id']), explode(',',"3,4,7,9,11,12,23,24,26,27,8"))): ?>
												****
												<?php else: ?>
												<?php echo $lists['user_name']; endif; else: ?>
												<?php echo $lists['user_name']; endif; ?>
											</td>
											<td>
												<?php if($adminid['group_id'] == 51): if(in_array(($lists['type_id']), explode(',',"3,4,7,9,11,12,23,24,26,27,8"))): ?>
												****
												<?php else: ?>
												<?php echo $lists['user_tel']; endif; else: ?>
												<?php echo $lists['user_tel']; endif; ?>
											</td>
											<td><?php echo $lists['type_name']; ?></td>
											<td><?php echo date('Y-m-d H:i:s',$lists['add_time']); ?></td>
											<td><?php echo $lists['user_develop_people']; ?></td>
											<td><?php echo $lists['user_server_people']; ?></td>
											<td><?php if(($lists['is_sub']==0)): ?>进入未提交<?php else: ?>已提交<?php endif; ?></td>

											<td>
												<?php if($is_sub == 1): ?>
												<a href="<?php echo url('wxadmint1/survey/shows'); ?>?survey_id=<?php echo $lists['survey_id']; ?>&user_id=<?php echo $lists['user_id']; ?>&is_sub=<?php echo $is_sub; ?>"><button class="btn btn-xs btn-danger"><i>查看</i></button></a>
												<?php endif; ?>
											</td>
										</tr>
									</tbody>
