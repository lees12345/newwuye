<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:77:"/home/wwwroot/only.1yuaninfo.com/./application/wxadmint1/view/survey/add.html";i:1635479599;s:71:"/home/wwwroot/only.1yuaninfo.com/application/wxadmint1/view/layout.html";i:1635484441;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="utf-8" />
...
<div id="Vue" class="Vue_div">
  ...
  <el-form ref="form" :model="form" label-width="100px">
    ...
    <div v-for="(item,index) in form.question" :key="item.key" class="questionDiv">
      <el-form-item :label= "'问题'+ item.label">
        <el-input v-model="item.title"></el-input>
      </el-form-item>

      <el-form-item :label="items.type" v-for="(items,index) in item.option" :key="item.key">
      ...
      <el-row style="margin-top:20px;">
        ...
        <el-col span="9">
          <el-button style="width:100%;background-color: #efefef;" @click.native="onQuestionOption(index)">+ 增加更多选项（点击此处）
          </el-button>
        </el-col>
      </el-row>
    ...
    <el-row style="margin-top:20px;margin-bottom: 50px;">
      ...
      <el-col span="16">
        <el-button style="width:100%" @click.native="onQuestionAdd()">+ 增加更多问题（点击此处）</el-button>
      </el-col>
    </el-row>


    <!-- <el-button size="mini" @click="onQuestionAdd">添加问题</el-button> -->
...
<script type="module">
  new Vue({
    ...
    methods: {
      onQuestionOption(index) {
        let QuestionOption = {}
        QuestionOption.name = ''
        QuestionOption.type = this.form.question[index].option.length+1
        this.form.question[index].option.push(QuestionOption)
      },
      onQuestionAdd() {
        let knowledge = {}
        knowledge.label = this.form.question.length+1;
        knowledge.title = ''
        knowledge.option = [
          {
            name: '',
          ...
          },
          {
            ...
            type: '4'
                ...
                this.form.question.push(knowledge)
      ...
      getStateAjaxFun(state) {
        $.ajax({
          url: 'http://zztong.1yuaninfo.com/index/personal/state_click',//地址
          // dataType: 'json',//数据类型
          type: 'get',//类型
          async: true,
          timeout: 2000,//超时
