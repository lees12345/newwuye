<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:85:"/home/wwwroot/only.1yuaninfo.com/./application/wxadmint1/view/stastics/new_index.html";i:1638254231;s:71:"/home/wwwroot/only.1yuaninfo.com/application/wxadmint1/view/layout.html";i:1637143029;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="utf-8" />
...
<head>
    ...
    <script src="http://zztong.admin.gszx77.com/public/static/admin/assets/js/distvue/vue.js"></script>
    <script src="http://zztong.admin.gszx77.com/public/static/admin/assets/js/distios/axios.min.js"></script>
    <link rel="stylesheet" href="http://zztong.admin.gszx77.com/public/static/admin/assets/js/element-ui/index.css">
    <!-- import JavaScript -->
    <script src="http://zztong.admin.gszx77.com/public/static/admin/assets/js/element-ui/index.js"></script>
...
<div id="Vue" class="budebuai" style="padding-bottom: 20px; position: relative;min-width: 850px; margin-left: 190px;">
    ...
    <div class="search_box flex-h">
        ...
        <button class="search_btn" @click="downOnSubmit" style="background:#4f99c6;color:#fff;">导出数据</button>
    </div>
    <div class="BarBox flex-v" style="overflow-y: auto;">
        ...
        <div v-if="dataObj && dataObj.show" class="showtitle">
            ...
            <div style="display: flex;justify-content: flex-end;">
                <div style=" margin-right: 10px;" ><a id="ree" href="<?php echo url('wxadmint1/stastics/newActivityLog'); ?>?type=0&start_time=<?php echo $start_time; ?>&end_time=<?php echo $end_time; ?>"><span style="color: #990099;font-weight: 500;">新注册人数:</span>{{dataObj.show.data.register}}</a></div>
                <div style=" margin-right: 10px;" ><a id="sii" href="<?php echo url('wxadmint1/stastics/newActivityLog'); ?>?type=1&start_time=<?php echo $start_time; ?>&end_time=<?php echo $end_time; ?>"><span style="color: #009900;font-weight: 500;">新签约人数:</span>{{dataObj.show.data.sign}}</a></div>
            </div>
        </div>
    ...
    new Vue({
        ...
        methods: {
            ...
            getDataFun(search) {
                axios({
                    methods: 'get',
                    url: this.url + '/wxadmint1/stastics/newGetChartData',
                    params: search
                    // {
            ...
            downtChartData(search) {
                this.loadingBar = false
                window.location.href = '/wxadmint1/stastics/downtChartData?start_time=' + search.start_time + '&end_time=' + search.end_time;

            },
            searchOnSubmit() {
                if(this.search.start_time && this.search.end_time){
                    // 第一次进入没有默认时间，因为js动态搜索，所以新注册人数及新签约人数没有对应的时间，所以拼接url+时间戳
                    var start_time = (new Date(this.search.start_time)).getTime()/1000
                    var end_time = (new Date(this.search.end_time)).getTime()/1000
                    var url = $('#ree').attr('href').substring(0,46) + '&start_time=' + start_time + '&end_time=' + end_time
                    var url2 = $('#sii').attr('href').substring(0,46) + '&start_time=' + start_time + '&end_time=' + end_time
                    $('#ree').attr('href',url)
                    $('#sii').attr('href',url2)
                }
                ///wxadmint1/stastics/newactivitylog.html?type=0&start_time=&end_time=
            ...
            downOnSubmit() {
                ...
                this.downtChartData(this.search)
            },
            detailsEvent(params) { // type 2注册3签约4结束
                ...
                if(params.seriesName != '成单比') {
                    ...
                    window.location.href= "http://zztong.admin.gszx77.com/wxadmint1/stastics/newGetMaterialUsersList?material="+ params.name + "&type=" + params.seriesName+'&start_time='+ start_time +'&end_time='+ end_time
                }

            },
            getMaterialDataFun() {
                this.materialList.push('全部')
            },
