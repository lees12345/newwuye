...
class CurlMuti extends Controller
{
    ...
    */
    ...
    public function send()
    {
        ...
        $curl = $ret = array();
        $handle = curl_multi_init(); #允许并行地处理批处理cURL句柄
    ...
    private function check_param($param = array())
    {
        ...
        if (isset($url)) {
            $url = trim($url);
            $url = stripos($url, 'https://') === 0 ? $url : NULL;
        }
    ...
    private function add_handle($handle, $param)
    {
        ...
        if ($param['method'] == 'POST') {
            ...
            curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($param['data'], JSON_UNESCAPED_UNICODE));
        }
        curl_multi_add_handle($handle, $curl); #$handle mutil 句柄
        return $curl;
    }
